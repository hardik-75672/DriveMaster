import React, { useState } from "react";
import plus from "../asset/plus.png";
import Modal from "@mui/material/Modal";
import { db, storage } from "../Firebase";
import { getDownloadURL, ref, uploadBytes } from "firebase/storage";
import { addDoc, collection } from "firebase/firestore";
import { useDispatch } from "react-redux";
import { logOutAsync } from "../redux/Slice/authSlice";
import { logOutApi } from "../redux/Slice/authApi";
import Upload from "../asset/upload-file.png";
import create from "../asset/create1.png";
import clock from "../asset/clock.png";
import folder from "../asset/folder.png";
import spam from "../asset/spam.png";
import starred from "../asset/starred.png";

// import firebase from "firebase";

const SideNavigate = ({ setIsCreateFolderModelOpen }) => {
  const [open, setOpen] = useState(false);
  const [uploading, setUploading] = useState(false);
  const [file, setFile] = useState(null);
  const dispatch = useDispatch();
  const handleFile = (e) => {
    if (e.target.files[0]) {
      setFile(e.target.files[0]);
    }
  };

  const handleUpload = async (e) => {
    e.preventDefault();
    setUploading(true);
    // storage
    //   .ref(`files/${file.name}`)
    //   .put(file)
    //   .then((snapshot) => {
    //     storage
    //       .ref("files")
    //       .child(file.name)
    //       .getDownloadURL()
    //       .then((url) => {
    //         db.collection("myfiles").add({
    //           // timestamp: firebase.firestore.FieldValue.serverTimestamp(),
    //           filename: file.name,
    //           fileURL: url,
    //           size: snapshot._delegate.bytesTransferred,
    //         });
    //         setUploading(false);
    //         setFile(null);
    //         setOpen(false);
    //       });
    //   });
    const image = ref(storage, `upload/images/${Date.now()}-${file.name}`);
    const uploadResult = await uploadBytes(image, file);
    const url = await getDownloadURL(ref(storage, uploadResult.ref.fullPath));
    await addDoc(collection(db, "files"), {
      imageURL: url,
      filepath: uploadResult.ref.fullPath,
      filename: file.name,
    });
    setUploading(false);
    setFile(null);
    setOpen(false);
  };
  const funci = () => {
    console.log("opoopopopopop");
    dispatch(logOutAsync());
  };
  return (
    <>
      <Modal open={open} onClose={() => setOpen(false)}>
        <div className=" relative top-1/2 bg-white w-400px mx-96 transform -translate-y-1/2 p-10  ">
          <form onSubmit={handleUpload}>
            <div className="text-center border-b border-gray-300 h-40">
              <h3>Select file you want to upload</h3>
            </div>
            <div>
              {uploading ? (
                <div className="bg-green-500 text-white m-20 text-center p-10 tracking-wider">
                  uploading.........
                </div>
              ) : (
                <>
                  <input
                    className=" bg-gray-300 text-black py-5 px-4 block mt-5"
                    type="file"
                    onChange={handleFile}
                  />
                  <input
                    className=" bg-red-500 text-black uppercase tracking-widest text-lg py-2 px-4 rounded cursor-pointer w-full"
                    type="submit"
                    value="Submit"
                  />
                </>
              )}
            </div>
          </form>
        </div>
      </Modal>
      <div>
        <div className="shadow-xl h-full p-3 space-y-2 w-60 bg-blue-50 text-gray-800">
          <button
            className="mx-5 p-3 px-4 shadow-xl bg-white rounded-lg"
            onClick={() => setOpen(true)}
          >
            ⬇ new
          </button>
          <div className="divide-y px-4 divide-gray-300">
            <ul className="pt-2 pb-4 space-y-1 text-sm ">
              <li className="text-gray-900 rounded-xl hover:bg-blue-200">
                <a
                  rel="noopener noreferrer"
                  href="#"
                  className="flex items-center p-2 space-x-3 rounded-md"
                >
                  <img
                    src={Upload}
                    className="w-5 h-5 fill-current text-gray-600"
                  />
                  <span>Upload File</span>
                </a>
              </li>
              <li className="text-gray-900 rounded-xl hover:bg-blue-200">
                <a
                  rel="noopener noreferrer"
                  href="#"
                  className="flex items-center p-2 space-x-3 rounded-md"
                >
                  <img
                    src={create}
                    className="w-5 h-5 fill-current text-gray-600"
                  />
                  <span>Create File</span>
                </a>
              </li>
              <li className="text-gray-900 rounded-xl hover:bg-blue-200">
                <button
                  rel="noopener noreferrer"
                  href="#"
                  className="flex items-center p-2 space-x-3 rounded-md"
                  onClick={() => {
                    setIsCreateFolderModelOpen(true);
                  }}
                >
                  <img
                    src={folder}
                    className="w-5 h-5 fill-current text-gray-600"
                  />

                  <span>Create Folder</span>
                </button>
              </li>
              <li className="text-gray-900 rounded-xl hover:bg-blue-200">
                <a
                  rel="noopener noreferrer"
                  href="#"
                  className="flex items-center p-2 space-x-3 rounded-md"
                >
                  <img
                    src={clock}
                    className="w-5 h-5 fill-current text-gray-600"
                  />
                  <span>Recent</span>
                </a>
              </li>
              <li className="text-gray-900 rounded-xl hover:bg-blue-200">
                <a
                  rel="noopener noreferrer"
                  href="#"
                  className="flex items-center p-2 space-x-3 rounded-md"
                >
                  <img
                    src={spam}
                    className="w-5 h-5 fill-current text-gray-600"
                  />
                  <span>Spam</span>
                </a>
              </li>
              <li className="text-gray-900 rounded-xl hover:bg-blue-200">
                <a
                  rel="noopener noreferrer"
                  href="#"
                  className="flex items-center p-2 space-x-3 rounded-md"
                >
                  <img
                    src={starred}
                    className="w-5 h-5 fill-current text-gray-600"
                  />
                  <span>Starred</span>
                </a>
              </li>
            </ul>
            <ul className="pt-4 pb-2 space-y-1 text-sm">
              <li className="text-gray-900 rounded-xl hover:bg-blue-200">
                <a
                  rel="noopener noreferrer"
                  href="#"
                  className="flex items-center p-2 space-x-3 rounded-md"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 512 512"
                    className="w-5 h-5 fill-current text-gray-600"
                  >
                    <path d="M245.151,168a88,88,0,1,0,88,88A88.1,88.1,0,0,0,245.151,168Zm0,144a56,56,0,1,1,56-56A56.063,56.063,0,0,1,245.151,312Z"></path>
                    <path d="M464.7,322.319l-31.77-26.153a193.081,193.081,0,0,0,0-80.332l31.77-26.153a19.941,19.941,0,0,0,4.606-25.439l-32.612-56.483a19.936,19.936,0,0,0-24.337-8.73l-38.561,14.447a192.038,192.038,0,0,0-69.54-40.192L297.49,32.713A19.936,19.936,0,0,0,277.762,16H212.54a19.937,19.937,0,0,0-19.728,16.712L186.05,73.284a192.03,192.03,0,0,0-69.54,40.192L77.945,99.027a19.937,19.937,0,0,0-24.334,8.731L21,164.245a19.94,19.94,0,0,0,4.61,25.438l31.767,26.151a193.081,193.081,0,0,0,0,80.332l-31.77,26.153A19.942,19.942,0,0,0,21,347.758l32.612,56.483a19.937,19.937,0,0,0,24.337,8.73l38.562-14.447a192.03,192.03,0,0,0,69.54,40.192l6.762,40.571A19.937,19.937,0,0,0,212.54,496h65.222a19.936,19.936,0,0,0,19.728-16.712l6.763-40.572a192.038,192.038,0,0,0,69.54-40.192l38.564,14.449a19.938,19.938,0,0,0,24.334-8.731L469.3,347.755A19.939,19.939,0,0,0,464.7,322.319Zm-50.636,57.12-48.109-18.024-7.285,7.334a159.955,159.955,0,0,1-72.625,41.973l-10,2.636L267.6,464h-44.89l-8.442-50.642-10-2.636a159.955,159.955,0,0,1-72.625-41.973l-7.285-7.334L76.241,379.439,53.8,340.562l39.629-32.624-2.7-9.973a160.9,160.9,0,0,1,0-83.93l2.7-9.972L53.8,171.439l22.446-38.878,48.109,18.024,7.285-7.334a159.955,159.955,0,0,1,72.625-41.973l10-2.636L222.706,48H267.6l8.442,50.642,10,2.636a159.955,159.955,0,0,1,72.625,41.973l7.285,7.334,48.109-18.024,22.447,38.877-39.629,32.625,2.7,9.972a160.9,160.9,0,0,1,0,83.93l-2.7,9.973,39.629,32.623Z"></path>
                  </svg>
                  <span>Settings</span>
                </a>
              </li>
              <li className="text-gray-900 rounded-xl hover:bg-blue-200">
                <a
                  rel="noopener noreferrer"
                  href="#"
                  className="flex items-center p-2 space-x-3 rounded-md"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 512 512"
                    className="w-5 h-5 fill-current text-gray-600"
                  >
                    <path d="M440,424V88H352V13.005L88,58.522V424H16v32h86.9L352,490.358V120h56V456h88V424ZM320,453.642,120,426.056V85.478L320,51Z"></path>
                    <rect width="32" height="64" x="256" y="232"></rect>
                  </svg>
                  <button onClick={funci}>Logout</button>
                </a>
              </li>
            </ul>
          </div>
        </div>
      </div>
    </>
  );
};

export default SideNavigate;
